  ...
  useEffect(() => {
    fetch('/api/leaderboard')
      ...
      .then((data: Participant[]) => {
        setParticipants(
          (data ?? []).filter((p) => p.participant_id !== session.participant_id)
        )
        setLoading(false)
      })
  }, [session])
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-sm space-y-5">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold">Copy a Friend's Bracket</h2>
          <button
            onClick={onClose}
        ...
        {loading ? (
          <div className="text-gray-500 text-sm animate-pulse py-2">Loading…</div>
